            ...
            <div className="relative z-10 space-y-12">
              <ScrollReveal>
                <div className="space-y-6 text-center">
                  <h1 className="mx-auto flex flex-col items-center justify-center gap-3 max-w-4xl text-[clamp(1.65rem,3.6vw,3rem)] font-display leading-[1.15] tracking-[-0.02em] text-[var(--color-text-primary)]">
                    <span className="whitespace-nowrap">שהמחשב יעשה לכם את <span className="text-[#c45c5c] line-through decoration-[#c45c5c]/40 decoration-2">הטכניקה והחישובים</span>.</span>
                    <span className="whitespace-nowrap">
                      תתמקדו <TypewriterEffect words={['ברציונל', 'בסמנטיקה', 'בנשמה']} />{' '}של הסטטיסטיקה.
                    </span>
                  </h1>
...
function TypewriterEffect({ words }: { words: string[] }): ReactElement {
  ...
  // Calculate the widest word to set fixed container width
  ...
  return (
    ...
    >
      {/* Notebook ruled underline */}
      <span
        className="absolute bottom-0 right-0 left-0 border-b-2 border-[var(--color-accent-brass)]/50"
        aria-hidden="true"
      />
      {/* Typed text */}
      <span className="font-'Times New Roman' font-bold text-[var(--color-accent-brass)] drop-shadow-sm">
        {text}
      </span>
      {/* Blinking cursor */}
